...
from Low_level_CV_PPL.models.modules.Attention.CBAM import CBAMBlock
...
class TransBlock(nn.Module):
    def __init__(self, dim, num_blocks, out_dim=128, dim_head=64, heads=8, window_size=16):
        super().__init__()
        self.dim = dim
...
class U_TransFormer(nn.Module):
    def __init__(
        ...
        heads, window_size, dim_head, num_blocks = map(partial(cast_tuple, depth=stages),
                                                       (heads, window_size, dim_head, num_blocks))

        for ind, heads, window_size, dim_head, num_blocks in zip(range(stages), heads, window_size, dim_head,
                                                                 num_blocks):
            is_last = ind == (stages - 1)
            self.downs.append(List([
                TransBlock(dim, num_blocks=num_blocks, dim_head=dim_head, heads=heads, window_size=window_size),
            ...
            if is_last:
                self.mid = TransBlock(dim=dim, num_blocks=num_blocks, dim_head=dim_head, heads=heads, window_size=window_size)
